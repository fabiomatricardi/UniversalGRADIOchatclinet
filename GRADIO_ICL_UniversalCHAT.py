...
def createClient():
    ...
    print('Insstance of OpenAI client done')
    return client

STOPS = ['<|im_end|>'] #'<|end_of_text|>' for granite

def getModelData(client):
    """
    Get model information from the OpenAI API call to the model
    INPUTS
    client -> OpenAI instance
    OUTPUTS
    MODELNAME -> str the GGUF filename string
    NCTX -> str the context window available in the server
    """
    try:
        mod = client.models.list()
        print(mod)
        MODELNAME =  mod.data[0].id.split('\\')[-1]  #'SmolLM2-360M-Instruct-GGUF'
        print(MODELNAME)
        NCTX = mod.data[0].meta['n_ctx_train']
        print(NCTX)
        print('model OK')
    except:
        MODELNAME =  'NOT DEFINED'
        NCTX = 'NOT DEFINED'  
        print('NO cONNECTION')    
    return MODELNAME, NCTX

client = createClient()
MODELNAME, NCTX = getModelData(client)
...
with gr.Blocks(fill_width=True,theme=theme) as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            RESTmodel = gr.Button("Check MODEL", variant='huggingface')
            def fixModelData(logfile):
                client = createClient()
                MODELNAME, NCTX = getModelData(client)
                writehistory(logfile,f'{datetime.datetime.now()}>> UPDATED MODEL CONNECTION to {MODELNAME} with {NCTX} context window\n================================================================================\n\n')
                str_title = f"""
            ## Advanced AI ChatBot Interface 
            - Running *{MODELNAME}* with llamaCPP-server 
            - Context window: {NCTX} tokens"""
                if MODELNAME == 'NOT DEFINED':
                    ErrMessage = 'No Model loaded in llama-server'
                else:
                    ErrMessage = f'Connected to llama-Server hosting {MODELNAME} with {NCTX} context window'
                return  str_title,ErrMessage
            RESTmodel.click(fixModelData,[LOGFILE],[title,ERRORbox])
